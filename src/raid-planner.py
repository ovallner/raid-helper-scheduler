...
def main():
    ...
    should_post, next_reset = determine_if_should_post()
    if not should_post or next_reset is None:
        return

    if next_reset['numberOfDaysRaiding'] == 2:
        print("Posting 2 raid days")
        handle_2_raid_reset(next_reset)
    elif next_reset['numberOfDaysRaiding'] == 3:
        print("Posting 3 raid days")
        handle_3_raid_reset(next_reset)
    else:
        print("Unsupported number of raid days")


def determine_if_should_post() -> tuple[bool, Optional[dict]]:
    ...
    num_raids = next_reset[0].get("numberOfDaysRaiding", 0)
    ...
    if today_datetime > twelve_hour_before_reset:
        ...
        print(f"{num_raids} Signups should be posted")
        return (True, next_reset[0])
    else:
        print(f"It is more than 12hrs until the reset")
        print(f"Signups should not be posted")
        return (False, next_reset[0])

def handle_2_raid_reset(next_reset: dict):
    next_reset_start = next_reset['resetStart']
    raid = next_reset['raid']
    ...
    for day in days_to_post:
        response = send_create_event_request(raid, day, next_reset_start, request_headers)
    ...
    insert_raid_post(num_signups_posted, next_reset)

def handle_3_raid_reset(next_reset: dict):
    next_reset_start = next_reset['resetStart']
    raid = next_reset['raid']
    request_headers = create_request_headers()
    ...
    num_signups_posted = 0
    for i in range(0, 3):
        response = send_create_event_request(raid, i, next_reset_start, request_headers)
        print(f"{response.status_code:}")
        if response.status_code != 200:
            print("Request did not finish with status code 200")
            break
...
def insert_raid_post(num_posts: int, next_reset: dict):
    next_raid_post = {
        "raid": next_reset["raid"],
        "resetId": next_reset["resetId"],
        "num_posts": num_posts,
        "resetStart": next_reset["resetStart"],
        "resetEnd": next_reset["resetStart"] + timedelta(days=3) - timedelta(seconds=1),
...
def send_create_event_request(raid: str, day_number: int, start_datetime: datetime, request_headers: dict) -> requests.Response:
    event_date = start_datetime + timedelta(days=day_number)
    event_json = create_event_json(raid, day_number, event_date)
...
def create_event_json(raid: str, day_number: int, start_date: datetime) -> dict:
    time = "7:00 PM"
    ...
    return {
        ...
        "time": time,
        "title": f"{raid} Reset night {day_number + 1}"
    }
